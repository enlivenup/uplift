...
import com.enlight.yaadle.uplift.LoginController;


@Component
public class OAuth2ClientContextFilter implements Filter, InitializingBean {
	private final Logger logger = LoggerFactory.getLogger(OAuth2ClientContextFilter.class);
	/**
	...
	public void doFilter(ServletRequest servletRequest,	ServletResponse servletResponse, FilterChain chain)	throws IOException, ServletException {
		...
		logger.info("URI log doFilter"+ request.toString());
		try {
	...
	protected void redirectUser(UserRedirectRequiredException e,HttpServletRequest request, HttpServletResponse response)	throws IOException {
		...
		String redirectUri = e.getRedirectUri();logger.info("URI log redirectUser"+ redirectUri);
		UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(redirectUri);
		logger.info("Build URI log redirectUser"+ redirectUri);
